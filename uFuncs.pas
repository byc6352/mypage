...
interface
uses
  windows,sysutils,strutils,classes,registry,System.Character;

  procedure IEEmulator(VerCode: Integer);
  function IsWin64: Boolean;
  function findfile(dir:string;match:string):string;
  function getNumberFromString(strIn: string;cStart,cEnd:char;var substr:string): string;
implementation
{-------------------------------------------------------------------------------
过程名:    getNumberFromString 从字符串中提取数字及双引号之间的字字符串
参数:      strIn: string;cStart,cEnd:char;var substr:string   1.字符串；2.开始字符；3.结束字符；4.开始与结束字符之间的字符串
返回值:    result:string  返回提取到的数字,无数字则为空
...
begin
  ...
  begin
    if(strIn[i]=cStart)and(bStart=false)then begin bStart:=true;continue;end;
    if not bStart then continue;
    if(strIn[i]=cEnd)then break;
    if IsNumber(strIn[i]) then    //isNumber--System.Character
    begin
      result := result + strIn[i];
    end;
    substr:=substr+strIn[i];
  end;
end;
{-------------------------------------------------------------------------------
过程名:    findfile 根据路径，匹配符查找文件
参数:      dir:string;match:string  1.路径名；2.匹配符
返回值:    result:string  返回文件名。
...
begin
  result:='';
  tmp:=dir+'\'+match;
  if findfirst(tmp,faanyfile,f)=0 then
  begin
    result:=dir+'\'+f.Name;
  end;
end;
